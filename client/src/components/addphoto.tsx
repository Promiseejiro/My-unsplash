...
const AddPhoto = ({ hideAddmodalHandler }: Func) => {
  ...
  const handleSelect = async (e: any) => {
    if (e.target) {
      try {
        ...
        setNewPhoto({
          ...newPhoto,
          uploadFilepath: res.data.filepath,
          uploading: true,
          disabled: false,
        });
      } catch (error) {
  ...
  const submitPhoto = async () => {
    ...
    if (newPhoto.label) {
      try {
        const res = await axios.post(`${baseurl}/photo`, {
          label: newPhoto.label,
          filepath: newPhoto.uploadFilepath
  ...
  return (
    <Modal
      modalContentent={
        <div className=" bg-[#fff] flex flex-col w-full p-4 rounded-[10px] mb-[9rem] mx-auto  md:w-[320px] inline-block ">
          ...
          {newPhoto.uploadFilepath && (
            <p className="break-all ">{newPhoto.uploadFilepath}</p>
          )}
          {!newPhoto.uploadFilepath && (
            <SearchBar
              onchange={handleChange}
              name="url"
              label="Valid photo url or select a photo"
            />
          )}
          {!newPhoto.uploadFilepath && (
          ...
          <div className="mt-4 flex ">
            <div className="flex justify-end w-full space-x-4">
              <Btn
                text="Cancel"
                onclickHandler={hideAddmodalHandler}
                bgcolor="#fff"
                color="black"
                loading={false}
                disabled={false}
              ...
              <Btn
                text="Submit"
                onclickHandler={submitPhoto}
                bgcolor="green"
                color="#fff"
                disabled={newPhoto.disabled}
                loading={newPhoto.loading}
                borderRadius={5}
                bordercolor="green"
              />
            </div>
